...
Sections 1 to 6 of the worksheet expect ideal data, so its preparation table tells students to switch the mode; section 7 on measurement uncertainty then switches back to real data. Its expected values its expected values were produced by running the page's own JavaScript (dukpy) and match the browser bit for bit. These are plain Word documents; edit them in Word. With U = 2000 V and d = 10 cm the settings give E = 20 kV/m, so F = 20 uN per nC and the measured points run from (0|0) to (10 nC|200 uN), exactly the diagonal of the printed grid.
